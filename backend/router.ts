...
const createTenantRouter: TenantRouterFactory = deps => {
  ...
  r.patch(
    ...
    asyncHandler(async (req: Request, res: Response) => {
      ...
      if (
        !action ||
        !['mark-read', 'mark-unread', 'archive', 'delete', 'restore'].includes(
          action,
        )
      ) {
        return sendError(
          res,
          400,
          'Valid action is required (mark-read, mark-unread, archive, delete, restore)',
        );
      }

      try {
        const contact = await db
          .table('tenant_contact_submissions')
          .where({ id })
          .first();
        ...
        switch (action) {
          case 'mark-read':
            ...
            logger.audit('contact_marked_read', {
              userId: user.id,
              contactId: id,
            });
            break;
          case 'mark-unread':
            updates.status = 'new';
            updates.readAt = null;
            logger.audit('contact_marked_unread', {
              userId: user.id,
              contactId: id,
            });
            break;
  ...
  r.post(
    ...
    asyncHandler(async (req: Request, res: Response) => {
      ...
      try {
        ...
        logger.info('Waitlist signup submitted', {
          id,
          email: sanitizedData.email,
        });
        ...
        const errorMsg = err instanceof Error ? err.message : String(err);
        const errorStack = err instanceof Error ? err.stack : undefined;
        logger.error('Failed to save waitlist signup', {
          error: errorMsg,
          stack: errorStack,
        });
